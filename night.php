...
<nav class="navbar navbar-expand-sm bg-pink" style="background-color:#ffeaf7;">
    <div class="container bg-pink">
        <ul class="navbar-nav align-items-center mx-auto" style="width: fit-content;">
            ...
            <li class="nav-item">
                <a class="nav-link font active mx-5" href="night.php">
                    <h6>Night💗</h6>
                </a>
            </li>
        </ul>
...
<div class='container flex-grow-1 outings-container'>
    <div class="row">
        <?php
        // Fetch all night
        $select_query = "SELECT * FROM night";
        $query_result = mysqli_query($conn, $select_query);

        // Fetch all planned night
        $select_plans = "SELECT night_id FROM plans";
        $query_result_plans = mysqli_query($conn, $select_plans);

        // Store planned night in an array
        $planned_night = [];
        while ($plan = mysqli_fetch_assoc($query_result_plans)) {
            $planned_night[] = $plan['night_id'];
        }
        ...
        while ($row = mysqli_fetch_array($query_result)) {
            ...
            $night_image = $row["night_image"];
            $is_in_plan = in_array($night_id, $planned_night);
            ?>
                ...
                <div class='m-4' style="border: 2px solid #000; border-radius:10px;">
                    <div
                        ...
                        <div
                            ...
                            <h4 class="ml-3"><?php echo $night_name; ?></h4>

                            <button class="mt-2 select-btn <?= !$is_in_plan ? 'btn-pink' : 'btn-disabled' ?>"
                                id="select-btn-<?= $night_id ?>" onclick="addToPlan(<?= $night_id ?>, 'night')"
                                <?= $is_in_plan ? 'disabled' : '' ?>>
